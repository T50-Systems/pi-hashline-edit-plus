...
import { detectLineEnding, normalizeToLF, stripBom } from "./edit-diff";
...
export function registerReadTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			throwIfAborted(signal);
			const rawText = stripBom(file.text).text;
			const originalEnding = detectLineEnding(rawText);
			const normalized = normalizeToLF(rawText);
			...
			const notes: string[] = [];
			if (originalEnding === "\r\n") {
				notes.push("Original line endings: CRLF. Hashline edit preserves CRLF on write.");
			}
			...
			if (file.hadUtf8DecodeErrors === true) {
				notes.push("Non-UTF-8 bytes shown as U+FFFD; editing rewrites the file as UTF-8.");
			}
			const previewText =
				notes.length > 0
